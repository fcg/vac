...
const response = await fetch("https://moving2canada.com/feed/");
const xml = await response.text();
const { entries } = await parseFeed(xml);

const mdbasedir = "./_feeds/_moving2canada/";
const shfilename = "moving2canada.sh";
const shfilearray = [];
let shcontent = "";
...
//   if (!pubdate.isAfter("2023-01-01")) return;
  ...
  const mdfilename = htmlfilename.replace(".html","_moving2canada.md");
  const cnmdfilename = mdfilename.replace("_moving2canada","_cn");
...
//   shfilearray.push(`curl -k -L -s --compressed ${pageurl} | pup ".m2c-post__content" | tee ${htmlfilename}\n`);
  ...
  shfilearray.push(`sed '/===/d;/----/d;/\\[!/d;/Something went wrong/d;/Load More Posts/d;/Loading/d;/data-lazy-fallback/d' "tmp_${mdfilename}" | sed 's/\\[\\([^][]*\\)\\]([^()]*)/\\1/g' | sed '/Related articles/,$d;/googletag/d;/Want to be the first/d;/Moving2Canada/d;/adsbygoogle/d;/！	!/d;/SHARE THIS ARTICLE/d;/Free Assessment/d;' | cat -s | tee ${mdfilename}`);
  ...
  shfilearray.push(`echo "\\nFCGvisa translated, © moving2canada All Rights Reserved." >> "../../_posts/${fcgmdfilename}"\n`);
...
const options = {};
  ...
  try {
    ...
    const article = $(".m2c-post__content").html();    
...
shfilearray.push("# rm -f *_moving2canada.md");
...
// sed -i '2,21d' *_moving2canada.md
// sed -i 's/\[\([^][]*\)\]([^()]*)/\1/g' *_moving2canada.md
// sed -i '/Tags:/,$d' *_moving2canada.md
// sed -i '/googletag/d' *_moving2canada.md
// sed -i '/moving2canada/d' *_moving2canada.md
// sed -i '/Discover if You Are Eligible for Canadian Immigration/d' *_moving2canada.md
// sed -i '/Visit CanadaVisa.com/d' *_moving2canada.md
// sed -i '/Schedule a Free/d;/Get a Free/d;' *.md

// sed '/Tags:/,$d;/googletag/d;/moving2canada/d;/Discover if You Are Eligible for Canadian Immigration/d;/Visit CanadaVisa.com/d;;'

// sed -i '2,22d;/googletag/d;/moving2canada/d' *_moving2canada.md
// sed -i 's/##* +//g;s/^\* +//g;' *_moving2canada.md

// sed -i 's/##* //g;' *_moving2canada.md
// sed -i 's/^\* //g;' *_moving2canada.md
