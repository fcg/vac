...
const response = await fetch("https://askmigration.com/feed/");
const xml = await response.text();
const { entries } = await parseFeed(xml);

const mdbasedir = "./_feeds/_askmigration/";
const shfilename = "askmigration.sh";
const shfilearray = [];
let shcontent = "";
...
entries.forEach(async (element) => {
  ...
  const mdfilename = htmlfilename.replace(".html","_askmigration.md");
  const cnmdfilename = mdfilename.replace("_askmigration","_cn");
  ...
  shfilearray.push(`echo "\\nFCGvisa translated, © AskMigration All Rights Reserved." >> "../../_posts/${fcgmdfilename}"\n`);
...
create: true,
  ...
  try {
    ...
    const article = $(".post-content").html();    
...
shfilearray.push("# rm -f *_askmigration.md");
...
// sed -i '2,21d' *_askmigration.md
// sed -i 's/\[\([^][]*\)\]([^()]*)/\1/g' *_askmigration.md
// sed -i '/Tags:/,$d' *_askmigration.md
// sed -i '/googletag/d' *_askmigration.md
// sed -i '/askmigration/d' *_askmigration.md
// sed -i '/Discover if You Are Eligible for Canadian Immigration/d' *_askmigration.md
// sed -i '/Visit CanadaVisa.com/d' *_askmigration.md
// sed -i '/Schedule a Free/d;/Get a Free/d;' *.md

// sed '/Tags:/,$d;/googletag/d;/askmigration/d;/Discover if You Are Eligible for Canadian Immigration/d;/Visit CanadaVisa.com/d;;'

// sed -i '2,22d;/googletag/d;/askmigration/d' *_askmigration.md
// sed -i 's/##* +//g;s/^\* +//g;' *_askmigration.md

// sed -i 's/##* //g;' *_askmigration.md
// sed -i 's/^\* //g;' *_askmigration.md
