...
const response = await fetch("https://canadianimmigrant.ca/feed");
const xml = await response.text();
const { entries } = await parseFeed(xml);

const mdbasedir = "./_feeds/_canadianimmigrant/";
const shfilename = "canadianimmigrant.sh";
const shfilearray = [];
let shcontent = "";
...
//   if (!pubdate.isAfter(yesterday)) return;
  ...
  const cnmdfilename = mdfilename.replace("_canadianimmigrant","_cn");
  ...
  shfilearray.push(`sed '/===/d;/----/d' "tmp_${mdfilename}" | sed 's/\\[\\([^][]*\\)\\]([^()]*)/\\1/g' | sed '/Tags:/,$d;/googletag/d;/Sponsor Content/d;/canadianimmigrant/d;/adsbygoogle/d;/！	!/d;/by The Canadian Magazine/d;/Visit CanadaVisa.com/d;' | cat -s | tee ${mdfilename}`);
  ...
  shfilearray.push(`echo "\\nFCGvisa translated, © canadianimmigrant All Rights Reserved." >> "../../_posts/${fcgmdfilename}"\n`);
...
create: true,
  ...
  try {
    ...
    const article = $(".article-post-content").html();    
...
shfilearray.push("# rm -f *_canadianimmigrant.md");
...
// sed -i '2,21d' *_canadianimmigrant.md
// sed -i 's/\[\([^][]*\)\]([^()]*)/\1/g' *_canadianimmigrant.md
// sed -i '/Tags:/,$d' *_canadianimmigrant.md
// sed -i '/googletag/d' *_canadianimmigrant.md
// sed -i '/canadianimmigrant/d' *_canadianimmigrant.md
// sed -i '/Discover if You Are Eligible for Canadian Immigration/d' *_canadianimmigrant.md
// sed -i '/Visit CanadaVisa.com/d' *_canadianimmigrant.md
// sed -i '/Schedule a Free/d;/Get a Free/d;' *.md

// sed '/Tags:/,$d;/googletag/d;/canadianimmigrant/d;/Discover if You Are Eligible for Canadian Immigration/d;/Visit CanadaVisa.com/d;;'

// sed -i '2,22d;/googletag/d;/canadianimmigrant/d' *_canadianimmigrant.md
// sed -i 's/##* +//g;s/^\* +//g;' *_canadianimmigrant.md

// sed -i 's/##* //g;' *_canadianimmigrant.md
// sed -i 's/^\* //g;' *_canadianimmigrant.md
