...
const response = await fetch("https://canadianvisa.org/feed/");
const xml = await response.text();
const { entries } = await parseFeed(xml);

const mdbasedir = "./_feeds/_canadianvisa/";
const shfilename = "canadianvisa.sh";
const shfilearray = [];
let shcontent = "";
...
//   if (!pubdate.isAfter("2023-01-01")) return;
  ...
  const mdfilename = htmlfilename.replace(".html","_canadianvisa.md");
  const cnmdfilename = mdfilename.replace("_canadianvisa","_cn");
  ...
  shfilearray.push(`sed '/===/d;/----/d' "tmp_${mdfilename}" | sed 's/\\[\\([^][]*\\)\\]([^()]*)/\\1/g' | sed '/Tags:/,$d;/googletag/d;/Sponsor Content/d;/canadianvisa/d;/adsbygoogle/d;/！	!/d;/SHARE THIS ARTICLE/d;/Visit CanadaVisa.com/d;' | cat -s | tee ${mdfilename}`);
  ...
  shfilearray.push(`echo "\\nFCGvisa translated, © canadianvisa All Rights Reserved." >> "../../_posts/${fcgmdfilename}"\n`);
...
create: true,
  ...
  try {
    ...
    const article = $(".article-section").html();    
...
shfilearray.push("# rm -f *_canadianvisa.md");
...
// sed -i '2,21d' *_canadianvisa.md
// sed -i 's/\[\([^][]*\)\]([^()]*)/\1/g' *_canadianvisa.md
// sed -i '/Tags:/,$d' *_canadianvisa.md
// sed -i '/googletag/d' *_canadianvisa.md
// sed -i '/canadianvisa/d' *_canadianvisa.md
// sed -i '/Discover if You Are Eligible for Canadian Immigration/d' *_canadianvisa.md
// sed -i '/Visit CanadaVisa.com/d' *_canadianvisa.md
// sed -i '/Schedule a Free/d;/Get a Free/d;' *.md

// sed '/Tags:/,$d;/googletag/d;/canadianvisa/d;/Discover if You Are Eligible for Canadian Immigration/d;/Visit CanadaVisa.com/d;;'

// sed -i '2,22d;/googletag/d;/canadianvisa/d' *_canadianvisa.md
// sed -i 's/##* +//g;s/^\* +//g;' *_canadianvisa.md

// sed -i 's/##* //g;' *_canadianvisa.md
// sed -i 's/^\* //g;' *_canadianvisa.md
