...
const response = await fetch("https://immigrationcoach.ca/feed/");
const xml = await response.text();
const { entries } = await parseFeed(xml);

const mdbasedir = "./_feeds/_immigrationcoach/";
const shfilename = "immigrationcoach.sh";
const shfilearray = [];
let shcontent = "";
...
entries.forEach(async (element) => {
  ...
  const mdfilename = htmlfilename.replace(".html","_immigrationcoach.md");
  const cnmdfilename = mdfilename.replace("_immigrationcoach","_cn");
  ...
  shfilearray.push(`sed '/===/d;/----/d' "tmp_${mdfilename}" | sed 's/\\[\\([^][]*\\)\\]([^()]*)/\\1/g' | sed '/FIND OUT IF YOU/,$d;/googletag/d;/Free Immigration Assessment/d;/immigrationcoach/d;/adsbygoogle/d;/！	!/d;/SHARE THIS ARTICLE/d;/Free Assessment/d;' | cat -s | tee ${mdfilename}`);
...
shfilearray.push("# rm -f *_immigrationcoach.md");
...
// sed -i '2,21d' *_immigrationcoach.md
// sed -i 's/\[\([^][]*\)\]([^()]*)/\1/g' *_immigrationcoach.md
// sed -i '/Tags:/,$d' *_immigrationcoach.md
// sed -i '/googletag/d' *_immigrationcoach.md
// sed -i '/immigrationcoach/d' *_immigrationcoach.md
// sed -i '/Discover if You Are Eligible for Canadian Immigration/d' *_immigrationcoach.md
// sed -i '/Visit CanadaVisa.com/d' *_immigrationcoach.md
// sed -i '/Schedule a Free/d;/Get a Free/d;' *.md

// sed '/Tags:/,$d;/googletag/d;/immigrationcoach/d;/Discover if You Are Eligible for Canadian Immigration/d;/Visit CanadaVisa.com/d;;'

// sed -i '2,22d;/googletag/d;/immigrationcoach/d' *_immigrationcoach.md
// sed -i 's/##* +//g;s/^\* +//g;' *_immigrationcoach.md

// sed -i 's/##* //g;' *_immigrationcoach.md
// sed -i 's/^\* //g;' *_immigrationcoach.md
