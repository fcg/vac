...
const response = await fetch("https://immigrants101.com/feed/");
const xml = await response.text();
const { entries } = await parseFeed(xml);

const mdbasedir = "./_feeds/_immigrants101/";
const shfilename = "immigrants101.sh";
const shfilearray = [];
let shcontent = "";
...
entries.forEach(async (element) => {
  ...
  const mdfilename = htmlfilename.replace(".html","_immigrants101.md");
  const cnmdfilename = mdfilename.replace("_immigrants101","_cn");
  ...
  shfilearray.push(`sed '/===/d;/----/d' "tmp_${mdfilename}" | sed 's/\\[\\([^][]*\\)\\]([^()]*)/\\1/g' | sed '/Tags:/,$d;/googletag/d;/Free Immigration Assessment/d;/immigrants101/d;/adsbygoogle/d;/！	!/d;/SHARE THIS ARTICLE/d;/Free Assessment/d;' | cat -s | tee ${mdfilename}`);
  ...
  shfilearray.push(`echo "\\nFCGvisa translated, © immigrants101 All Rights Reserved." >> "../../_posts/${fcgmdfilename}"\n`);
...
create: true,
  ...
  try {
    ...
    const article = $(".entry-content").html();    
...
shfilearray.push("# rm -f *_immigrants101.md");
...
// sed -i '2,21d' *_immigrants101.md
// sed -i 's/\[\([^][]*\)\]([^()]*)/\1/g' *_immigrants101.md
// sed -i '/Tags:/,$d' *_immigrants101.md
// sed -i '/googletag/d' *_immigrants101.md
// sed -i '/immigrants101/d' *_immigrants101.md
// sed -i '/Discover if You Are Eligible for Canadian Immigration/d' *_immigrants101.md
// sed -i '/Visit CanadaVisa.com/d' *_immigrants101.md
// sed -i '/Schedule a Free/d;/Get a Free/d;' *.md

// sed '/Tags:/,$d;/googletag/d;/immigrants101/d;/Discover if You Are Eligible for Canadian Immigration/d;/Visit CanadaVisa.com/d;;'

// sed -i '2,22d;/googletag/d;/immigrants101/d' *_immigrants101.md
// sed -i 's/##* +//g;s/^\* +//g;' *_immigrants101.md

// sed -i 's/##* //g;' *_immigrants101.md
// sed -i 's/^\* //g;' *_immigrants101.md
