...
const response = await fetch("http://www.canadim.com/feed/");
const xml = await response.text();
const { entries } = await parseFeed(xml);

const mdbasedir = "./_feeds/_canadim/";
const shfilename = "canadim.sh";
const shfilearray = [];
let shcontent = "";
...
//   if (!pubdate.isAfter("2023-01-01")) return;
  ...
  const mdfilename = htmlfilename.replace(".html","_canadim.md");
  const cnmdfilename = mdfilename.replace("_canadim","_cn");
  ...
  shfilearray.push(`sed '/===/d;/----/d' "tmp_${mdfilename}" | sed 's/\\[\\([^][]*\\)\\]([^()]*)/\\1/g' | sed '2,4d;/Tags:/,$d;/googletag/d;/Immigration Assessment/d;/canadim/d;/immigration assessment/d;/！	!/d;/SHARE THIS ARTICLE/d;/Canadim can help/d;' | cat -s | tee ${mdfilename}`);
  ...
  shfilearray.push(`echo "\\nFCGvisa translated, © canadim All Rights Reserved." >> "../../_posts/${fcgmdfilename}"\n`);
...
create: true,
  ...
  try {
    ...
    const article = $(".o-container-col").html();    
...
shfilearray.push("# rm -f *_canadim.md");
...
// sed -i '2,21d' *_canadim.md
// sed -i 's/\[\([^][]*\)\]([^()]*)/\1/g' *_canadim.md
// sed -i '/Tags:/,$d' *_canadim.md
// sed -i '/googletag/d' *_canadim.md
// sed -i '/canadim/d' *_canadim.md
// sed -i '/Discover if You Are Eligible for Canadian Immigration/d' *_canadim.md
// sed -i '/Visit CanadaVisa.com/d' *_canadim.md
// sed -i '/Schedule a Free/d;/Get a Free/d;' *.md

// sed '/Tags:/,$d;/googletag/d;/canadim/d;/Discover if You Are Eligible for Canadian Immigration/d;/Visit CanadaVisa.com/d;;'

// sed -i '2,22d;/googletag/d;/canadim/d' *_canadim.md
// sed -i 's/##* +//g;s/^\* +//g;' *_canadim.md

// sed -i 's/##* //g;' *_canadim.md
// sed -i 's/^\* //g;' *_canadim.md
